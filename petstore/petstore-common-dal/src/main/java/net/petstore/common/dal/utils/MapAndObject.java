...
/**
 * 混合两个对象为一个Map对象 MapAndObject.get()方法将在两个对象取值,Map如果取值为null,则再在Bean中取值ֵ
 *
 * @author badqiu
 */
public class MapAndObject implements Map {
  ...
  /**
   * Instantiates a new Map and object.
   *
   * @param map the map
   * @param bean the bean
   */
  ...
  /**
   * Gets from map or bean.
   *
   * @param key the key
   * @return the from map or bean
   */
    ...
    private static Object getBeanPropertyValue(Object bean, String propertyName) {
      ...
      try {
        ...
        throw new IllegalStateException(
            "cannot get property value by property:"
                + propertyName
                + " on class:"
                + bean.getClass(),
            e);
