...
public class Paginator implements java.io.Serializable {
   ...
   * 是否有下一页
   *
   * @return 下一页标识 boolean
   */
  public boolean isHasNextPage() {
    return (page + 1 <= getTotalPages());
  }

  /** 开始行，可以用于oracle分页使用 (1-based)。 @return the start row */
  ...
  /** 结束行，可以用于oracle分页使用 (1-based)。 @return the end row */
  public int getEndRow() {
    return page > 0 ? Math.min(pageSize * page, getTotalItems()) : 0;
  }

  /** offset，计数从0开始，可以用于mysql分页使用(0-based) @return the offset */
  public int getOffset() {
    return page > 0 ? (page - 1) * getPageSize() : 0;
  }

  /** limit，可以用于mysql分页使用(0-based) @return the limit */
  public int getLimit() {
    if (page > 0) {
   ...
   *
   * @param slidersCount the sliders count
   * @return integer [ ]
   */
  ...
  private static int computeLastPageNumber(int totalItems, int pageSize) {
    ...
    int result =
        (int) (totalItems % pageSize == 0 ? totalItems / pageSize : totalItems / pageSize + 1);
    if (result <= 1) {
      result = 1;
    }
    return result;
  ...
  private static Integer[] generateLinkPageNumbers(
      int currentPageNumber, int lastPageNumber, int count) {
    int avg = count / 2;
