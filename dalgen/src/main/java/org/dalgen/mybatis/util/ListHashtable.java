...
/**
 * @author MF1180 Hashtable that maintains the input order of the data elements - Note only put,
 *         putall, clear and remove maintains the ordered list
 */
public class ListHashtable extends Hashtable {
  ...
  /**
   * @param aKey
   * @returns calculate position at which the first key is greater otherwise return -1 if no key can
   *          be found which is greater
   */
  ...
  /** This method reorders the ListHashtable only if the keys used are integer keys. */
  ...
  /**
   * This method looks through the list of values and returns the key associated with the value..
   * Otherwise if not found, null is returned
   *
   * @param aValue
   * @return
   */
