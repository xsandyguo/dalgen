 ...
 * These inflections will be useful in code generators that convert things like database table names
 * into Java class names.
 *
 * <p>
 * The <code>getInstance()</code> method returns a singleton instance of this class with a default
 * set of rules, which can then be customized. Rules added during customization will take precedence
 * over the standard ones. Use the <code>addIrregular()</code>, <code>addPlural()</code>,
 * <code>addSingular()</code>, and <code>addUncountable()</code> methods to add additional rules ot
 * the default ones.
 *
 * <p>
 * <strong>IMPLEMENTATION NOTE</strong> - The default implementation is intended to be functionally
 * compatible with the <code>Inflector::inflections</code> class in Ruby on Rails. The
 * <code>gsub()</code> method on Ruby strings matches regular expressions anywhere in the input.
 * However, nearly all of the actual patterns used in this module use <code>$</code> at the end to
 * match the end of the input string (so that only the last word in a multiple word phrase will be
 * singularized or pluralized). Therefore, the Java versions of the regular expressions have been
 * modified to capture all text before the interesting characters at the end, and emit them as part
 * of the result, so that the entire string can be matched against a pattern once.
 */
public class Inflector {

  // ------------------------------------------------------------ Constructors

  /** The singleton instance returned by the default <code>getInstance()</code> method. */
  private static transient Inflector instance     = null;

  // -------------------------------------------------------- Static Variables
  /**
   * List of <code>Replacer</code>s for performing replacement operations on matches for plural
   * words.
   */
  private List                       plurals      = new LinkedList();
  /**
   * List of <code>Replacer</code>s for performing replacement operations on matches for addSingular
   * words.
   */
  ...
  /**
   * Return a fully configured {@link Inflector} instance that can be used for performing
   * transformations.
   */
  ...
  /**
   * Convert strings to <code>EmbeddedCamelCase</code>. Embedded underscores will be removed.
   *
  ...
  /**
   * Convert word strings consisting of lower case letters and underscore characters between words
   * into <code>embeddedCamelCase</code> or <code>EmbeddedCamelCase</code>, depending on the <code>
   * lower</code> flag. Embedded underscores will be removed. Embedded '/' characters will be
   * replaced by '.', making this method useful in converting path-like names into fully qualified
   * classnames.
   *
   * <p>
   * <strong>IMPLEMENTATION DIFFERENCE</strong> - The Rails version of this method also converts '/'
   ...
   *
   * @param word Word to be converted
   * @param flag Flag indicating that the initial character should be lower cased instead of upper
   *        cased
   */
  ...
  /**
   * Create and return a simple class name that corresponds to a addPlural table name. Any leading
   * schema name will be trimmed.
   *
   * <table border="1" width="100%">
   * <tr>
   * <th>Input</th>
  ...
  /**
   * Create and return a foreign key name from a class name, separating the "id" suffix with an
   * underscore.
   */
  ...
  /**
   * Create and return a foreign key name from a class name, optionally inserting an underscore
   * before the "id" portion.
   *
   * <table border="1" width="100%">
   * <tr>
   * <th>Input</th>
   ...
   * @param underscore Flag indicating whether an underscore should be emitted between the class
   *        name and the "id" suffix
   */
  ...
  /**
   * Capitalize the first word in a lower cased and underscored string, turn underscores into
   * spaces, and string any trailing "_id". Like <code>titleize()</code>, this is meant for creating
   * pretty output, and is not intended for code generation.
   *
   * <table border="1" width="100%">
   * <tr>
   * <th>Input</th>
  ...
  /**
   * Turn a number into a corresponding ordinal string used to denote the position in an ordered
   * sequence.
   *
   * <table border="1" width="100%">
   * <tr>
  ...
  /**
   * Convert the simple name of a model class into the corresponding name of a database table, by
   * uncamelizing, inserting underscores, and pluralizing the last word.
   *
   * <table border="1" width="100%">
   * <tr>
   * <th>Input</th>
  ...
  /**
   * Capitalize all the words, and replace some characters in the string to create a nicer looking
   * title. This is meant for creating pretty output, and is not intended for code generation.
   *
   * <table border="1" width="100%">
   * <tr>
   * <th>Input</th>
  ...
  /**
   * The reverse of <code>camelize()</code>, makes an underscored form from the expression in the
   * string. Changes "." to "/" to convert fully qualified class names into paths.
   *
   * <table border="1" width="100%">
   * <tr>
   * <th>Input</th>
  ...
  /**
   * Add the addSingular and addPlural forms of words that cannot be converted using the normal
   * rules.
   *
   ...
   * By default, matches will be case insensitive.
   *
  ...
  /**
   * Internal class that uses a regular expression matcher to both match the specified regular
   * expression to a specified word, and (if successful) perform the appropriate substitutions.
   */
  private class Replacer {
    ...
    /**
     * Return <code>true</code> if our regular expression pattern matches the specified input. If it
     * does, save necessary state information so that the <code>replacement()</code> method will
     * return appropriate results based on the <code>rule</code> specified to our constructor.
     *
    ...
    /**
     * Return a replacement string based on the <code>rule</code> that was specified to our
     * constructor. This method <strong>MUST</strong> only be called when the <code>matches()</code>
     * method has returned <code>true</code>.
     */
