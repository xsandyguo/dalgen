...
public class StatementCreatorUtils {
  ...
  public static void setParameterRandomValue(PreparedStatement ps, int parameterIndex, int sqlType,
  ...
  public static void setRandomParamsValueForPreparedStatement(String executeSql,
      PreparedStatement ps, List<SqlParameter> params) throws SQLException {
    ...
    for (int parameterIndex = 1; parameterIndex <= count; parameterIndex++) {
      ...
      if (index < sqlParameters.length) {
        ...
        StatementCreatorUtils.setParameterRandomValue(ps, parameterIndex, parameter.getSqlType(),
            parameter.getSize());
      } else {
  ...
  // }
  // } catch (Throwable ex) {
  // log.debug("Could not check database or driver name,cause:"+ ex);
  // }
  // if (useSetObject) {
  ...
  // || (sqlTypeToUse == Types.CLOB &&
  // isStringValue(inValueToUse.getClass()))) {
  ...
  // } else if (inValueToUse instanceof Calendar) {
  // Calendar cal = (Calendar) inValueToUse;
  // ps.setTimestamp(paramIndex, new
