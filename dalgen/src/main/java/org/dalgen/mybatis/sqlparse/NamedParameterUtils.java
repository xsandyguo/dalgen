 ...
 *
 * <p>
 * Helper methods for named parameter parsing. Only intended for internal use within Spring's JDBC
 * framework.
 *
...
public abstract class NamedParameterUtils {
    ...
    while (i < statement.length) {
      ...
      if (c == ':' || c == '&' || c == '#' || c == '$') {
        ...
        if (j - i > 1) {
          ...
          String removedPrefixAndSuffixParameter = removePrefixAndSuffix(c, parameter, sql); // add
                                                                                             // by
                                                                                             // badqiu
   ...
   * will be grouped and enclosed with parantheses. This allows for the use of "expression lists" in
   * the SQL statement like:<br>
   * select id, name, state from table where (name, age) in (('John', 35), ('Ann', 50))
   *
   * <p>
   * The parameter values passed in are used to determine the number of placeholder to be used for a
   * select list. Select lists should be limited to 100 or fewer elements. A larger number of
   * elements is not guaramteed to be supported by the database and is strictly vendor-dependent.
   *
