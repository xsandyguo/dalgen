...
import org.dalgen.mybatis.provider.db.table.DatabaseMetaDataUtils;
import org.dalgen.mybatis.util.IOHelper;
...
public class SqlParseHelper {
  ...
  static Pattern     fromRegex  =
      Pattern.compile("(\\sfrom\\s+)([,\\w]+)", Pattern.CASE_INSENSITIVE);
  static Pattern     join       =
      Pattern.compile("(join\\s+)(\\w+)(as)?(\\w*)", Pattern.CASE_INSENSITIVE);
  static Pattern     update     =
      Pattern.compile("(\\s*update\\s+)(\\w+)", Pattern.CASE_INSENSITIVE);
  static Pattern     insert     =
      Pattern.compile("(\\s*insert\\s+into\\s+)(\\w+)", Pattern.CASE_INSENSITIVE);
  ...
  public static Set<NameWithAlias> getTableNamesByQuery(String sql) {
    ...
    if (m.find()) {
      ...
      if (from.indexOf(',') >= 0) {
        ...
        // join的多表
        String removedFrom =
            StringHelper.removeMany(from.toLowerCase(), "inner", "full", "left", "right", "outer");
