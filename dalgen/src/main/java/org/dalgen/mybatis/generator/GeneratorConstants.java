...
public enum GeneratorConstants {
  ...
  GENERATOR_INCLUDES("generator_includes", null, ""), GENERATOR_EXCLUDES("generator_excludes", null,
      ""),

  GENERATOR_SOURCE_ENCODING("generator_sourceEncoding", "UTF-8",
      "生成器模板的输入编码,默认UTF-8"), GENERATOR_OUTPUT_ENCODING("generator_outputEncoding", "UTF-8",
          "生成器模板生成的文件编码,默认UTF-8"),

  GENERATOR_REMOVE_EXTENSIONS("generator_removeExtensions", ".ftl,.vm",
      "生成器模板自动删除的模板后缀,默认值为: .ftl,.vm"),

  GG_IS_OVERRIDE("gg_isOverride", "false", "用于控制模板默认是否覆盖输出文件,默认值是false"), TABLE_REMOVE_PREFIXES(
      "tableRemovePrefixes", null,
      "需要移除的表名前缀,示例值: t_,v_"), TABLE_NAME_SINGULARIZE("tableNameSingularize", "false",
          "用于控制表名是否自动转换为单数, 如 customers ==> customer. 默认值是 false"), GENERATOR_SQL_RESULTCLASS_SUFFIX(
              "generator_sql_resultClass_suffix", "Result",
              "generate by sql中查询结果使用的后缀，默认值为: Result"),

  JDBC_USERNAME("jdbc_username", null, "数据源:用户名"), JDBC_PASSWORD("jdbc_password", null,
      "数据源:密码"), JDBC_DRIVER("jdbc_driver", null, "数据源:驱动"), JDBC_URL("jdbc_url", null,
          "数据源:URL"), JDBC_SCHEMA("jdbc_schema", null,
              "数据源:schema"), JDBC_CATALOG("jdbc_catalog", null, "数据源:catalog"),
