...
/**
 * gg变量,生成器模板控制器,用于模板中可以控制生成器执行相关控制操作 如: 是否覆盖目标文件
 *
 * <pre>
 * 使用方式:
...
public class GeneratorControl {
  ...
  private boolean isOverride          =
      GeneratorProperties.getBoolean(GeneratorConstants.GG_IS_OVERRIDE);
  ...
  private String  sourceEncoding;                                       // no pass //?
                                                                        // 难道process两次确定sourceEncoding
  ...
  public void generateFile(String outputFile, String content, boolean append) {
    try {
      ...
      if (deleteGeneratedFile) {
        log.info("[delete gg.generateFile()] file:" + realOutputFile + " by template:"
            + getSourceFile());
        new File(realOutputFile).delete();
      } else {
        File file = new File(realOutputFile);
        FileHelper.parentMkdir(file);
        log.info("[gg.generateFile()] outputFile:" + realOutputFile + " append:" + append
      ...
      throw new RuntimeException(
          "gg.generateFile() occer error,outputFile:" + outputFile + " caused by:" + e, e);
    }
  }
  ...
  public String insertAfter(String compareToken, String str) throws IOException {
    String content =
        IOHelper.readFile(new File(outRoot, outputFile).getAbsoluteFile(), sourceEncoding);
    if (StringHelper.isBlank(content)) {
      throw new IllegalArgumentException(
          new File(outRoot, outputFile).getAbsolutePath() + " is blank");
    }
