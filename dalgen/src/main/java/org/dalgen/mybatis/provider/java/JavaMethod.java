 ...
 *
 * <p>
 * Copyright 2008 [pomer], Inc. All rights reserved. Website: http://www.pomer.org.cn/
 */
package org.dalgen.mybatis.provider.java;
...
import org.dalgen.mybatis.provider.java.MethodParameter.JavaSourceFileMethodParametersParser;
import org.dalgen.mybatis.typemapping.JavaImport;
...
public class JavaMethod {
  ...
  public List<FieldMethodInvocation> getFieldMethodInvocationSequences() {
    if (StringHelper.isNotBlank(clazz.getMavenJavaSourceFileContent())) {
      try {
        JavaMethodInvokeSequencesParser cmd =
            new JavaMethodInvokeSequencesParser(this, clazz.getMavenJavaSourceFileContent());
        cmd.execute();
        return cmd.getMethodInvokeSequences();
      } catch (Exception e) {
        log.warn("getFieldMethodInvocationSequences() occer error on method:" + method.toString(),
            e);
  ...
  public static class JavaMethodInvokeSequencesParser {
    ...
    private List<FieldMethodInvocation> methodInvokeFlows      =
    ...
    public List<FieldMethodInvocation> getMethodInvokeSequences() {
      if (executed) {
        ...
        throw new IllegalStateException(
            "please invoke execute() method before getMethodInvokeFlows()");
      }
    }
