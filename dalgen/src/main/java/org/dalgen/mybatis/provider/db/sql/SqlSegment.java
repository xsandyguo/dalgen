...
import org.dalgen.mybatis.sqlparse.NamedParameterUtils;
...
public class SqlSegment {
  ...
  public Set<SqlParameter> getParams(Sql sql) {
    ...
    for (String paramName : getParamNames()) {
      ...
      if (p == null)
        throw new IllegalArgumentException("not found param on sql:" + parsedIncludeSql
            + " with name:" + paramName + " for sqlSegment:" + id); // 是否不该扔异常
      result.add(p);
    }
    return result;
  }
  ...
  public List<String> getParamNames() {
    ParsedSql parsedSql = NamedParameterUtils.parseSqlStatement(parsedIncludeSql); // FIXME 没有执行替换?为
                                                                                   // :name的动作
