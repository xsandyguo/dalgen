...
import org.dalgen.mybatis.sqlparse.SqlTypeChecker;
 ...
 */
public class Sql {
  public static String        MULTIPLICITY_ONE    = "one";                            // select查询回一条记录,
                                                                                      // selectOne()
  public static String        MULTIPLICITY_MANY   = "many";                           // select查询回一个List,
                                                                                      // selectList()
  ...
  private String _getResultClass() {
    ...
    if (isColumnsInSameTable()) {
      Collection<NameWithAlias> tableNames = SqlParseHelper.getTableNamesByQuery(executeSql);
      Table t = SqlFactory.getTableFromCache(tableNames.iterator().next().getName()); // FIXME
                                                                                      // 自定义的className将不会起作用,因为不是从同一个cache取的对象
      return t.getClassName();
    } else {
      if (operation == null)
        return null;
      return StringHelper.makeAllWordFirstLetterUpperCase(StringHelper.toUnderscoreName(operation))
          + GeneratorProperties.getProperty(GeneratorConstants.GENERATOR_SQL_RESULTCLASS_SUFFIX);
    }
  }
  ...
  public String getIbatisSql() {
    return StringHelper.isBlank(ibatisSql)
        ? SqlParseHelper.convert2NamedParametersSql(getSql(), "#", "#")
        : ibatisSql;
  }
  ...
  public String getMybatisSql() {
    return StringHelper.isBlank(mybatisSql)
        ? SqlParseHelper.convert2NamedParametersSql(getSql(), "#{", "}")
        : mybatisSql;
  }
